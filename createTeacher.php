...
if($create = filter_input(INPUT_POST, 'create')){
    ...
    $subject        = filter_input(INPUT_POST, 'subject',FILTER_VALIDATE_INT) or besked("fejl", "Ikke gyldig vin");
...
?>
    <div class="loginBox">
        ...
        <form class="form" action="?create=1" method="post" enctype="multipart/form-data">
            <input type="text" name="name" placeholder="Vinens Navn" required>
            <select name="subject" required>
                <?=getSubjects($link)?>
            </select>
            <input type="file" name="image">
            <input type="submit" name="create" value="Tilføj Vin">
        </form>
    </div>
